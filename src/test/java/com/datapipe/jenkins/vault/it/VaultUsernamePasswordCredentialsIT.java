...
public class VaultUsernamePasswordCredentialsIT {
    ...
    public void setup() throws Exception {
        ...
        globalConfig.setConfiguration(new VaultConfiguration(VAULT_ADDRESS, GLOBAL_CREDENTIALS_ID_1, true, "mynamespace", 60));
    ...
    public void shouldRetrieveCorrectCredentialsFromVault() throws Exception {
        ...
        store.addCredentials(Domain.global(), new VaultUsernamePasswordCredentialImpl(null, "cloudfoundry", "secret/cloudfoundry", null, null, "1", "Test Credentials"));
        ...
        assertThat(log, containsString("peterparker:spiderman"));
    ...
    public void shouldRetrieveCorrectCredentialsFromVaultWithCustomKeys() throws ExecutionException, InterruptedException, IOException {
        ...
        CredentialsStore store = CredentialsProvider.lookupStores(Jenkins.getInstance()).iterator().next();
        store.addCredentials(Domain.global(), new VaultUsernamePasswordCredentialImpl(null, "custom", "secret/custom", "name", "alias", "1", "Test Credentials"));
    ...
    public void shouldFailWhenMissingRequestingMissingCredentialsFromVault() throws ExecutionException, InterruptedException, IOException {
        ...
        CredentialsStore store = CredentialsProvider.lookupStores(Jenkins.getInstance()).iterator().next();
        store.addCredentials(Domain.global(), new VaultUsernamePasswordCredentialImpl(null, "cloudfoundry", "does/not/exist", null, null, "1", "Test Credentials"));
